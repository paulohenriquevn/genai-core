...
class XlsConnector(DataConnector):
    """
    Excel connector with semantic layer support.
    
    Extends the standard Excel connector to utilize metadata information
    for better data interpretation and transformation, and semantic schema
    for view construction.
    Supports reading Excel files with multiple sheets.
    """
    ...
    def connect(self) -> None:
        """
        Load the Excel file or directory of Excel files into memory.
        """
        try:
            ...
            if os.path.isdir(path):
                ...
                logger.info(f"Connecting to Excel directory: {path} with pattern {pattern}")
                ...
                for xls_file in self.xls_files:
                    try:
                        ...
                        logger.info(f"Loading Excel file: {file_name}")
                        ...
                        logger.error(f"Error loading Excel file {file_name}: {str(e)}")
                ...
                logger.info(f"Connecting to Excel: {path}")
                ...
                if sheet_name == 'all':
                    ...
                    # Concatenate all sheets if requested
                    if self.config.params.get('auto_concat', True):
                        try:
                            self.data = pd.concat(self.dataframes.values(), ignore_index=True)
                        except Exception as e:
                            logger.warning(f"Could not concatenate sheets: {str(e)}")
                            # Use the first sheet as fallback
                            self.data = next(iter(self.dataframes.values()))
                    else:
                        # By default, use the first sheet as the main data
                        self.data = next(iter(self.dataframes.values()))
                    ...
                    self._connected = True
                    logger.info(f"Successfully connected to Excel: {path}")
            ...
            self._connected = False
            error_msg = f"Error connecting to Excel {self.config.params.get('path')}: {str(e)}"
    ...
    def read_data(self, query: Optional[str] = None) -> pd.DataFrame:
        """
        Read data from the Excel file or directory of Excel files, optionally applying an SQL query.
        ...
        try:
            ...
            if not query:
                if (self.is_directory or sheet_name == 'all') and self.config.params.get('return_dict', False):
            ...
            # If it's a directory or multiple sheets, we need special logic for queries
            if (self.is_directory or self.config.params.get('sheet_name') == 'all') and self.dataframes:
                return self._execute_query_on_multiple_dfs(query)
            ...
            error_msg = f"Error reading data from Excel: {str(e)}"
            logger.error(error_msg)
            raise DataReadException(error_msg) from e

    def _execute_query_on_multiple_dfs(self, query: str) -> pd.DataFrame:
        """
        Execute an SQL query on multiple DataFrames (multiple files or sheets).
        ...
        for df_name, df in self.dataframes.items():
            ...
            if self.is_directory:
                table_name = os.path.splitext(df_name)[0]
            else:
                # For sheets, use the sheet name directly
                table_name = df_name
                
            table_name = ''.join(c if c.isalnum() else '_' for c in table_name)
            df.to_sql(table_name, conn, if_exists='replace', index=False)
            logger.info(f"Registered {'file' if self.is_directory else 'sheet'} {df_name} as table {table_name}")
        
        # Also register the concatenated DataFrame for simple queries
        if self.data is not None:
            combined_table = f"xls_data_{self.config.source_id}"
            self.data.to_sql(combined_table, conn, if_exists='replace', index=False)
            
            # Replace generic table references in the query
            modified_query = query.replace("FROM xls", f"FROM {combined_table}")
        else:
    ...
    def close(self) -> None:
        """
        Free resources. For Excel, just clear the data reference.
        """
        self.data = None
        self.dataframes = {}
        self.xls_files = []
        self._connected = False
        logger.info(f"Excel connection closed: {self.config.params.get('path')}")
    ...
    def is_connected(self) -> bool:
        ...
        """
        if self.is_directory or len(self.dataframes) > 1:
    ...
    def get_available_tables(self) -> List[str]:
        ...
        Returns:
            List[str]: List of available file names/sheet names/tables
        """
